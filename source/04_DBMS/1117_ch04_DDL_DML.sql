...
(1) DCL:
 ...
 권한박탁      REVOKE       사용자계정삭제 DROP USER
 트렌젝션 명령어(ROLLBACK, COMMIT)
(2) DDL:
 테입블생성 CREATE TABLE, 테이블구조변경 ALTER TABLE, 테이블삭제 DROP TABLE
(3) DML:CRUD
 ...
 CREATE TABLE BOOK(
  ...
  BOOKNAME  VARCHAR2(20), -- BOOKNAME필드 타입은 문자 20바이트(한글 1자=3바이트)
  PUBLISHER VARCHAR2(20),
  RDATE     DATE,        -- RDATE필드 타입은 DATE형(날짜+시간)
  PRICE     NUMBER(8,2), -- PRICE 필드타입은 숫자 전체 8자리 중 소숫점 2자리
...
CREATE TABLE BOOK(
  ...
  -- DEPT와 유사한 DEPT01 생성: DEPTNO(숫자2-PK), DNAME(문자14_, LOC(문자 13)
CREATE TABLE DEPT01(
...
---1. 붙임 구조를 만족하는 MY_DATA 테이블을 생성하시오. 다만, ID가 PRIMARY KEY입니다.
...
-- 3. TO-CHAR 내장 함수를 이용하여 입력한 자료를 위의 도표와 같은 형식으로 출력하는 SQL문을 작성해 주세요.
SELECT 
    ID,
    NAME,
    USERID,
...
--- (2) 영구적으로 반영
COMMIT;
--7. SALARY가 15000.00이하인 사람의 급여를 15000.00으로 변경하라.
--- SALARY가 15000.00이하인 사람의 급여를 15000.00으로 변경
UPDATE MY_DATA
SET SALARY = 15000.00
